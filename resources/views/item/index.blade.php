...
@section('content')
    <div class="table-responsive">
            ...
            <tbody>
                ...
                <tr id="agent_{{$cus->id}}">
                    ...
                    <td style="padding: 0; text-align: center;"><div>
                            <a href="{{route('edit_item', ['id' => $cus->id])}}" class="label label-default"><i class="glyphicon glyphicon-edit"></i></a>
                            <a href="javascript:void(0);" title="Delete {{$cus->name}}" 
                                data-should-be-removed="#agent_{{$cus->id}}"
                                data-delete-url="{{route('delete_item', ['id' => $cus->id])}}" 
        ...
        <a class="btn btn-primary" href="{{route('new_item')}}">New</a>
        <div style="float: right;">{!! $items->render() !!}</div>
    </div>
@endsection